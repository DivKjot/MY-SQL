...
WITH CTE AS (
SELECT pizza_id ,COUNT(pizza_id) AS num_of_orders,
CASE 
...
WHERE FIND_IN_SET(pizza_recipes.toppings, '1,2,3,4,5,6,8,10') ;



SELECT FIND_IN_SET('apple', 'apple,banana,orange') AS position;
SELECT FIND_IN_SET(3, '1,2,3,4,5') AS position;

ALTER TABLE pizza_recipes
MODIFY toppings TEXT;

UPDATE pizza_recipes 
SET toppings = (SELECT SPLIT_STR((toppings), ',', 1));

SELECT * FROM pizza_recipes 
WHERE pizza_recipes.toppings IN (FIND_IN_SET('1,2,3,4,5,6,8,10', ','));